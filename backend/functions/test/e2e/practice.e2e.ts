...
// Both pick kinds, written once (D-72): what is being checked is that a board
// crosses the wire as artwork with no country on it, and that is the same
// promise whether the artwork is eight flags or eight silhouettes.
for (const [kind, art] of [["flagPick", "flag"], ["shapePick", "shape"]] as const) {
  test(`FR-8.7: a ${kind} crosses the wire as an index, and the options carry no country`, async () => {
    const start = ok(await ana.call("startPractice", { kind }), "startPractice");
    assert.equal(start.item.prompt.kind, kind);
    ...
    // pieces of artwork and nothing that says which is whose. The subject is
    // server-side, so the check is against the stored session, not a guess.
    ...
    // Only artwork: every option is exactly one field and nothing else.
    for (const o of start.item.prompt.options) assert.deepEqual(Object.keys(o), [art]);
