...
test("listTournaments shows the group's tournaments and the presets the form needs", async () => {
  ...
  assert.deepEqual(v.presets.map((p: Any) => p.id), ["quintal", "capitais", "mistura", "bandeiras", "economia", "qual-bandeira", "qual-silhueta", "quem-nasceu-onde", "liga", "mata-mata", "suico", "chave-dupla"]);
