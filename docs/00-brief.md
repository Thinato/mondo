...
| D-45 | **The shipped daily is not refactored onto the challenge-kind interface.** | `lib/round.ts` is live, tested and playing. A dozen lines of glue duplicated in the `shape` kind is the cheaper mistake; consolidation is Phase 4+ if ever. |
