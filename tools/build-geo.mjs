...
// FR-9.9 — the continents a player may narrow practice to. `world-countries`'
// own `region`, which is the pt-BR school notion of a continent exactly: the
// Americas are one, not two. Listed here so a sixth (`Antarctic`, which no
// country in the pool carries) cannot arrive unnoticed and become a filter
// nothing matches.
...
for (const code of codes) {
  ...
  // The continent, for FR-9.9's practice filter. SERVER-ONLY, like the capital
  // and the centroid: a client-side code→continent map would not name an answer
  // on its own, but it would cut a silhouette's field from 194 to 12 in one
  // lookup, and the client has no use for one — the player picks the continent,
  // so the server already knows which it is.
  if (!REGIONS.includes(meta.region)) errors.push(`${code}: unknown region "${meta.region}"`);
...
write(
  ...
  JSON.stringify(
    {
      "//": "GENERATED by tools/build-geo.mjs — do not edit. Server-only: centroids feed distance/bearing (§7), tiers feed the schedule, capitals are the `capital` challenge kind's prompt, regions are practice's continent filter (FR-9.9). The client copy is site/data/countries.min.json and deliberately lacks all of this — a capital→country map on the client would name the answer in one lookup (SEC-2).",
      sources,
...
console.log(`  continents: ${REGIONS.map((r) => `${r} ${countries.filter((c) => c.region === r).length}`).join(" · ")}`);
